...
function addComponentsFromContext(componentsContext, componentsToHydrate) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var customEvents = component.R_;
    var scope = component.K_;
    var bubblingDomEvents = component.Q_;

    component.M_ = undefined; // We don't use `delete` to avoid V8 deoptimization
    component.T_ = undefined; // We don't use `delete` to avoid V8 deoptimization
    ...
    componentsToHydrate.push([id, // 0 = id
    typeName, // 1 = type
    input, // 2 = input
    extra // 3
    ]);
  ...
  var nestedContexts = componentsContext._O_;
...
exports.g_ = function getInitComponentsCode(out, componentDefs) {
  ...
  return componentGlobalKey + "=(window." + componentGlobalKey + "||[]).concat(" + safeJSON(warp10.stringify(initComponentsData)) + ")||" + componentGlobalKey;
};
